...
import { Map, WFS } from "./components/ol-react";
import praga from "./spider.png";
import anotacao from "./anemometer_mono.png";
...
function App() {
  ...
  async function salvar() {
    const talhaoIDs = await WFS.insertFeatures(
      GEOSERVER_WFS_URL,
      "agro",
      "talhao",
      drawTalhaoSource.getFeatures()
    );
    const pragaIDs = await WFS.insertFeatures(
      GEOSERVER_WFS_URL,
      "agro",
      "praga",
      drawPragaSource.getFeatures()
    );
    const anotacaoIDs = await WFS.insertFeatures(
      GEOSERVER_WFS_URL,
      "agro",
      "anotacao",
  ...
  return (
    <div>
      <Map height={"100vh"} width={"100vw"} options={options}>
        <Map.Toolbar>
          <Map.Toolbar.Button
